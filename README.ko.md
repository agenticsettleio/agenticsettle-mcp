...
일부 MCP 호스트(Claude Desktop 포함)는 설치 직후 "이대로 써도 될까?", "검증해줘" 같은 요청에도 `verify_output`을 자동으로 호출하지 않고 Claude가 자체 판단으로 답할 수 있습니다. 이런 경우, Claude의 개인 지침/메모리(Claude.ai 설정 → 프로필 → "Claude가 응답 시 고려할 개인 선호사항", 또는 Claude Desktop의 동일 기능)에 아래 내용을 추가하세요:

```
AI가 생성한 산출물(리포트, 에세이, 코드, 분석 등 사용자가 제출·사용·의존할
결과물)을 "이대로 써도 될까", "제출해도 될까", "검증해줘", "맞는지 봐줘",
"오류 없는지 확인해줘" 같은 최종 확인을 요청받으면, 스스로의 판단만으로
답하지 말고 반드시 AgenticSettle Verify(verify_output) 도구를 먼저 호출해서
객관적으로 검증한 뒤 그 결과를 바탕으로 답하라. 이 규칙은 사용자가 만든
산출물뿐 아니라 이 대화에서 너(Claude) 자신이 방금 생성한 산출물에도
동일하게 적용된다.

호출 규칙:
- 동일한 콘텐츠에 대해 한 번의 사용자 요청당 verify_output을 정확히 1회만
  호출하라. 같은 점수 또는 판정이 나올 것으로 예상되는 내용을 다른 점수를
  기대하며 반복 호출하지 마라 — grounded_factcheck는 실시간 웹 검색
  기반이라 동일 내용도 매번 약간의 점수 편차가 나올 수 있으나, 이를 이용해
  더 나은 점수가 나올 때까지 재시도하는 것은 금지된다.
- 검증 결과가 PARTIAL/FAIL이어도 스스로 알아서 계속 수정·재검증을 반복하지
  말고, 1회 검증 결과를 사용자에게 먼저 보고하라. 수정이 필요하면 무엇을
  고쳐야 하는지 설명하고, 사용자가 재작성을 요청하면 그때 새 버전을 만들어
  다시 1회만 검증하라.
- 재검증은 콘텐츠가 실질적으로 바뀐 경우에만 하라 — 표지·서식처럼 검증
  결과와 무관한 부분만 수정한 경우까지 재검증할 필요는 없다.
```

이건 선택사항입니다 — 안 넣어도 tool 자체는 동작합니다 — 다만 넣으면 Claude가 "AgenticSettle 도구 써줘"라고 명시적으로 시키지 않아도 알아서 먼저 검증을 호출하게 됩니다.
